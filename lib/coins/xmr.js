...
function Coin(data){
    ...
    this.getPortBlockHeaderByID = function(port, blockId, callback){
        ...
        } else if (port == 18144 || port == 18146 || port == 18148) { // XTM
    ...
    this.getPortAnyBlockHeaderByHash = function(port, blockHash, is_our_block, callback){
        ...
        if (port == 11898) {
        ...
        } else if (port == 18144 || port == 18146 || port == 18148) {
            ...
            const miner_tx_hash = port == 19950 ? "" : (body.result.miner_tx_hash == "" ? body.result.block_header.miner_tx_hash : body.result.miner_tx_hash);
            ...
            if (is_our_block && miner_tx_hash) global.support.rpcPortWalletShort(port + 1, "get_transfer_by_txid", {"txid": miner_tx_hash}, function (body2) {
                ...
                if (port == 17750) {
                   ...
                   body2.result.transfers.forEach(function(transfer) { if (transfer.asset_type === "ZEPH") reward = transfer.amounts[0]; });
                }
	...
	global.coinFuncs.getPortLastBlockHeader(port, function (is_err, body) {
	    if (is_err) return callback(is_err, body);
            if (port == 18144 || port == 18146 || port == 18148) {
    ...
    this.getPortLastBlockHeaderMM = function(port, callback, no_error_report) { // with merge mining header
		...
		// we use 18146 port instead 18144 port here since 18146 can be served locally
                global.coinFuncs.getPortLastBlockHeader(18146, function(is_err, body2) {
                    if (is_err) return callback(is_err, body);
                    body.mm = body2;
                    return callback(null, body);
                }, no_error_report);
    ...
    this.getPortBlockTemplate = function(port, callback) {
		...
		method: 'getblocktemplate',
           ...
           global.support.rpcPortDaemon(port, 'GetNewBlockTemplateWithCoinbases', { algo: {pow_algo: 0 }, coinbases: [{address: global.config.pool["address_18146"], value: 1, stealth_payment: true, revealed_value_proof: true, coinbase_extra: [] }] }, function(body){
                if (body && body.result) {
                   body.result.block.difficulty = parseInt(body.result.miner_data.target_difficulty);
                   body.result.block.reward = parseInt(body.result.miner_data.reward);
                   return callback(arr2hex(body.result.block));
                } else return callback(null);
            });

        } else if (port == 18146) {
            global.support.rpcPortDaemon(port, 'GetNewBlockTemplateWithCoinbases', { algo: {pow_algo: 2 }, coinbases: [{address: global.config.pool["address_" + port.toString()], value: 1, stealth_payment: true, revealed_value_proof: true, coinbase_extra: [] }] }, function(body) {
                if (body && body.result) {
            ...
            global.support.rpcPortDaemon(port, 'GetNewBlockTemplateWithCoinbases', { algo: {pow_algo: 3 }, coinbases: [{address: global.config.pool["address_18146"], value: 1, stealth_payment: true, revealed_value_proof: true, coinbase_extra: [] }] }, function(body) {
                if (body && body.result) {
                    const result = body.result;
                    // The format of blobtemplate_blob is:
                    // 2 bytes (generated by miner from login job xn field) | 6 bytes nonce (generated on miner size) | 32 bytes (mining hash) |
                    return callback({ blocktemplate_blob: arr2hex(result.merge_mining_hash),
                                      reserved_offset: 0,
                                      bt_nonce_size: 8,
                                      difficulty: parseInt(result.miner_data.target_difficulty),
                                      reward: parseInt(result.miner_data.reward),
    ...
    this.blobTypeXTM_C = function(blob_type_num) { return blob_type_num == 107; }
    ...
    this.convertBlob = function(blobBuffer, port) {
        ...
        if (this.blobTypeXTM_C(blob_type_num)) return Buffer.from(blobBuffer);
        let blob;
        try {
	...
	this.xtm_block          = template.xtm_block;
        ...
        if (global.coinFuncs.blobTypeRvn(port_blob_num) || global.coinFuncs.blobTypeRtm(port_blob_num) || global.coinFuncs.blobTypeKcn(port_blob_num) || global.coinFuncs.blobTypeXTM_T(port_blob_num) || global.coinFuncs.blobTypeXTM_C(port_blob_num) || is_dero) {
...
//        if      ("rx/graft" in algos_perf)      coin_perf["GRFT"] = algos_perf["rx/graft"];
        ...
        if      ("c29" in algos_perf) {
            if (!("XWP" in coin_perf))  coin_perf["XWP"]  = algos_perf["c29"];
            if (!("XTNC" in coin_perf)) coin_perf["XTNC"] = algos_perf["c29"];
            if (!("XMV" in coin_perf))  coin_perf["XMV"]  = algos_perf["c29"];
            if (!("TUBE" in coin_perf)) coin_perf["TUBE"] = algos_perf["c29"];
            coin_perf["XTM-C"] = algos_perf["c29"];
        }
...
//            case 34568:
    ...
    this.c29 = function(header, ring, port) {
        switch (port) {
            case 18148: return multiHashing.c29(header, ring);  // XTM-C
    ...
    this.blobTypeStr = function(port, version) {
	    ...
	    case 10225: return "raptoreum";       // BTRM
            ...
            case 19994: return "cryptonote_arq";  // ARQ
