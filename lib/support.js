...
function jsonRequest(host, port, data, callback, path, timeout) {
	...
	rejectUnauthorized: false, // do not verify self-signed TLS certificates
        timeout: timeout,
        headers: {
            "Content-Type": "application/json",
            "Accept":       "application/json",
...
module.exports = function () {
    return {
	    ...
	    if (debugging == true) console.log("[DEBUG] rpcDaemon method: " + method + " params: " + params + " port: " + port);
	    ...
	    if (debugging == true) console.log("[DEBUG] rpcPortDaemon method: " + method + " params: " + params + " port: " + port);
            rpc(global.config.daemon.address, port, method, params, callback, 30*1000);
        },
        rpcPortDaemon2: function (port, method, params, callback) {
	    if (debugging == true) console.log("[DEBUG] rpcPortDaemon2 method: " + method + " params: " + params + " port: " + port);
