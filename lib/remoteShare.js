...
function messageHandler(message) {
    if (typeof message.raw_shares === "number"){
    ...
    try {
        ...
        switch (msgData.msgType) {
            ...
            case global.protos.MESSAGETYPE.ALTBLOCK:
                global.database.storeAltBlock(msgData.exInt, msgData.msg, function(data){
                    if (!data){
                        return res.status(400).end();
                    } else {
                        return res.json({'success': true});
...
function storeShares(){
    if (Object.keys(shareData).length > 0){
        console.log('Storing ' + Object.keys(shareData).length + ' shares');
