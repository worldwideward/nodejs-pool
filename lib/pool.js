...
function get_new_eth_extranonce_id() {
    if (!freeEthExtranonces.length) {
        const err_str = threadName + "Pool server " + global.config.hostname + " has overlow extranonce of " + (16 - global.coinFuncs.uniqueWorkerIdBits) + " bits";
...
function eth_extranonce(id) {
    return id === null ? null : pad_hex(((id << global.coinFuncs.uniqueWorkerIdBits) + global.coinFuncs.uniqueWorkerId).toString(16), 2);
};
...
function sendToWorkers(data) {
    ...
    console.debug("[DEBUG] Send to Workers - DATA: ", data)
    ...
    try {
	    ...
	    console.error("[ERROR] No such worker")
    }
}
...
function templateUpdate3(coin, port, coinHashFactor, isHashFactorChange, body_block_template) {
    const template = process_rpc_template(body_block_template, coin, port, coinHashFactor, isHashFactorChange);
    console.debug("[DEBUG] " + threadName + "New block template found at " + template.height + " height");
...
function templateUpdate2(coin, port, isHashChange, coinHashFactor, isHashFactorChange, body_header, times_failed) {
	...
	console.debug("[DEBUG] TU2 - body block template: " + body_block_template)
	    ...
	    if (times_failed <= 2) {
		setTimeout(templateUpdate2, 500, coin, port, isHashChange, coinHashFactor, isHashFactorChange, body_header, times_failed);
        ...
        if (isHashChange || (isTimeChange && (!isRewardCheckReady || body_bt.expected_reward !== lastBlockReward[coin])) || isRewardChange) {
            ...
            return templateUpdate3(coin, port, coinHashFactor, isHashFactorChange, body_block_template);
        }
    });
}
...
function templateUpdate(coin, repeating) {
	...
	if (!newCoinHashFactor[coin]) {
		...
		lastBlockHeight[coin] = body.height;
	    	...
	    	console.debug("[DEBUG] Performing call to 'templateUpdate2'")
                templateUpdate2(coin, port, isHashChange, coinHashFactor, isHashFactorChange, body);
            } else if (isHashFactorChange) {
	    	console.debug("[DEBUG] Performing call to 'coinHashFactorUpdate'")
                coinHashFactorUpdate(coin, coinHashFactor);
            }
	    console.debug("[DEBUG] Call to SetTimeout with templateUpdate")
            if (repeating === true) setTimeout(templateUpdate, DAEMON_POLL_MS, coin, repeating);
        } else {
...
function setNewCoinHashFactor(isHashFactorChange, coin, coinHashFactor, check_height) {
    ...
    if (isHashFactorChange) {
        ...
        if (cluster.isMaster) console.log("[INFO] " + threadName + strLogPrefix + " with hash factor changed to " + lastCoinHashFactorMM[coin]);
        ...
        if (cluster.isMaster) console.log("[INFO] " + threadName + strLogPrefix + " with the same " + lastCoinHashFactorMM[coin] + " hash factor");
    ...
    if (protoVersion === 1) {
        this.getCoinJob = function (coin, params) {
            ...
            if (!this.proxy || isExtraNonceBT) {
                ...
                const isXTM_C = global.coinFuncs.blobTypeXTM_C(blob_type_num);
...
function readUInt64BE(buf, offset = 0) {
 const hi = BigInt(buf.readUInt32BE(offset));     // high 32 bits
  const lo = BigInt(buf.readUInt32BE(offset + 4)); // low 32 bits
  const val = (hi << 32n) | lo;
  return val.toString(10); // return as decimal string
}
...
function submit_block(miner, job, blockTemplate, blockData, resultBuff, hashDiff, isTrustedShare, isParentBlock, portUsedToSubmit, submit_blockCB) {
    ...
    let reply_fn = function (rpcResult, rpcStatus, port, submit_blockCB) {
        ...
        // Success! Submitted a block without an issue.
            ...
            const get_block_id = function(cb) {
                ...
                } else if (global.coinFuncs.blobTypeXTM_C(blob_type_num)) {
    ...
    if (blockTemplate.port == 11898) {
        ...
        global.support.rpcPortDaemon(blockTemplate.port, "SubmitBlock", blockTemplate.xtm_block, std_reply_fn);
...
function processShare(miner, job, blockTemplate, params, processShareCB) {
    ...
    let verifyShare = function(verifyShareCB) {
        if (global.coinFuncs.blobTypeXTM_C(blob_type_num)) {
            const header = Buffer.concat([bignum(params.nonce, 16).toBuffer({endian: 'big', size: 8}), blockTemplate.buffer]);
            if (global.coinFuncs.c29(header, params.pow, port)) {
                report_miner_share(miner, job);
                return processShareCB(invalid_share(miner));
            }
            const c29_packed_edges_buff = global.coinFuncs.c29_packed_edges(params.pow, blob_type_num);
	    job.c29_packed_edges = Array.from(Buffer.from(c29_packed_edges_buff, "hex"));
            const resultBuff = global.coinFuncs.c29_cycle_hash(c29_packed_edges_buff);
            return verifyShareCB(hashBuffDiff(resultBuff), resultBuff, header, false, true);

        } else if (global.coinFuncs.blobTypeGrin(blob_type_num)) {
            ...
            const resultBuff = global.coinFuncs.c29_cycle_hash(global.coinFuncs.c29_packed_edges(params.pow, blob_type_num));
                ...
                if (coin !== false) {
                    ...
                    if ( global.coinFuncs.blobTypeRvn(blob_type_num) ||
                        ...
                        if (new_id !== null) {
                            socket.eth_extranonce_id = new_id;
                            miner.eth_extranonce     = eth_extranonce(new_id);
			    let job = miner.getCoinJob(coin, params);
            ...
            const nonce_sanity_check = function(blob_type_num, params) {
                if (global.coinFuncs.blobTypeGrin(blob_type_num)) {
                    if (global.coinFuncs.blobTypeXTM_C(blob_type_num)) {
                        ...
                        if (!params.nonce.toLowerCase().startsWith(miner.eth_extranonce)) return false;
                    } else {
            ...
            processShare(miner, job, blockTemplate, params, function(shareAccepted) {
                ...
                if (miner.protocol === "grin") {
                    sendReply(null, "ok");
                            ...
                            global.coinFuncs.blobTypeEth(blob_type_num) ||
                            global.coinFuncs.blobTypeErg(blob_type_num) ||
    			    global.coinFuncs.blobTypeXTM_C(blob_type_num)
                          ) {
                    sendReply(null, true);
                } else {
...
if (cluster.isMaster) {
    ...
    console.debug("[DEBUG] Set initial block template")
    templateUpdate("");
    ...
    while (maxId) { maxId >>= 1; ++ global.coinFuncs.uniqueWorkerIdBits; }
    freeEthExtranonces = [...Array(1 << (16 - global.coinFuncs.uniqueWorkerIdBits)).keys()];
    console.log(threadName + "Starting pool worker with " + global.coinFuncs.uniqueWorkerId + " unique id and " + global.coinFuncs.uniqueWorkerIdBits + " reserved bits");
