...
function registerPool() {
    ...
    if (debugging == true) console.debug("[DEBUG] Pool registered");
}

// Master/Slave communication Handling
function messageHandler(message) {
    if (debugging == true) console.debug("[DEBUG] Handle message: " + message)
...
function sendToWorkers(data) {
    ...
    if (debugging == true) {
	    console.debug("[DEBUG] Send to Workers - Block hashing blob: ", data.blockhashing_blob)
...
function templateUpdate3(coin, port, coinHashFactor, isHashFactorChange, body_block_template) {
    ...
    if (debugging == true) { console.debug("[DEBUG] " + threadName + "New block template found at " + template.height + " height"); }
...
function getCoinJobParams(coin) {
    ...
    console.log("[INFO] Got Coin Job Params: " + util.inspect(params, { depth: null }))
    return params;
};
...
let wallet_debug = global.config.debug_wallets;
// Do not ban from these IPs
let ip_whitelist = {};

function getTargetHex(diff, size) { return pad_hex(baseDiff.div(diff).toBuffer({endian: 'little', size: size}).toString('hex'), size); };

function getRavenTargetHex(diff) { return pad_hex((baseRavenDiff / diff).toString(16), 32); };
...
function recordShareData(miner, job, isTrustedShare, blockTemplate) {
    ...
    if (isTrustedShare) {
        ...
        if (debugging == true) console.debug("[DEBUG] " + threadName + "Accepted trusted share at difficulty: " + job.difficulty + "/" + job.rewarded_difficulty + " from: " + miner.logString);
    } else {
        process.send({type: 'normalShare'});
        if (debugging == true) console.debug("[DEBUG] " + threadName + "Accepted valid share at difficulty: " + job.difficulty + "/" + job.rewarded_difficulty + " from: " + miner.logString);
    }
...
if (cluster.isMaster) {
    ...
    console.log("[DEBUG] Pool Main If")
    ...
    if (debugging == true) console.debug("[DEBUG] Set initial block template");
    templateUpdate("");
    ...
    if (debugging == true) {
	console.log("[DEBUG] Pool Main Else")
