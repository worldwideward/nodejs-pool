...
function templateUpdate2(coin, port, isHashChange, isMMHashChange, coinHashFactor, isHashFactorChange, body_header, times_failed) {
    // Ports for ETH and ETC --> Ethereum isn't mined anymore.
...
function templateUpdate(coin, repeating) {
    ...
    console.log("[DEBUG] Coin: " + coin)
    console.log("[DEBUG] Repeating: " + repeating)
    console.log("[DEBUG] Coin hashfactor: " + coinHashFactor)
    if (coinHashFactor) {
      // Don't merge mine by default - might add this back in later
	...
	if (!newCoinHashFactor[coin]) {
	    ...
	    console.log("[DEBUG] Is Merge Mining Hash change: " + isMMHashChange)
